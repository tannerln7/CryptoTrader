...
Procedure: Activate `.venv` and capture `REPO_ROOT=$(pwd)` from the repo checkout. Create `data/systemd/<instance>/config.yaml` from `config/config.example.yaml`, edit it for the instance you plan to run, and validate it with `market-recorder validate-config --config "${REPO_ROOT}/data/systemd/<instance>/config.yaml"`. Run `sudo ./ops/install/install.sh --instance <instance>` without `--enable` first. If that runtime config already exists, the installer pre-fills `/etc/market-recorder/<instance>.env` with `MARKET_RECORDER_CONFIG=${REPO_ROOT}/data/systemd/<instance>/config.yaml`; otherwise it falls back to `config/config.example.yaml`. Inspect `/etc/market-recorder/<instance>.env` and update it only when you need a different repo checkout, repo-local Python, or config path. Before the first start, verify `sudo -u market-recorder test -x "${REPO_ROOT}"`, `sudo -u market-recorder test -x "${REPO_ROOT}/.venv/bin/python"`, and `sudo -u market-recorder test -r "${REPO_ROOT}/data/systemd/<instance>/config.yaml"`. If the repo access check fails, verify execute and traverse permissions on every parent directory in the repo path, not just on the repo directory itself. Refresh your group membership with `newgrp market-recorder` or by logging out and back in. If you want the unit enabled on future boots, rerun `sudo ./ops/install/install.sh --instance <instance> --enable`; this enables autostart only and does not start the service. Use `market-recorder start` to launch the installed service, `market-recorder status` to confirm the control socket and effective runtime paths, and `market-recorder health`, `stop`, or `restart` for normal lifecycle control.
