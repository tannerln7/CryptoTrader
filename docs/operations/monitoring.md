...
Log Surface Notes: `data/service/recorder-service.log` captures the background worker's stdout and stderr for the CLI-managed path and is written line-buffered (the worker subprocess is launched with `PYTHONUNBUFFERED=1`, matching the systemd unit). The recorder is intentionally quiet during steady-state ingestion and only writes startup messages, exception tracebacks, and shutdown notices to this file, so an empty or short log during a healthy run is expected; rely on `market-recorder status`, `market-recorder health`, and `report-data-quality` for ongoing signal. For a systemd-managed instance the same output is captured by journald, so use `journalctl -u market-recorder@<instance>.service` instead of the repo-scoped log file.
